...
import { Search, MapPin, Phone, ShieldCheck, Menu, X, ChevronDown, Home, Info, Layers, BookOpen, PhoneCall, ArrowRight, ShoppingCart, Settings, History, Users } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const navLinks: { id: ActivePage; label: string; icon: React.FC<{ className?: string }> }[] = [
    ...
    ...(authUser
      ? [
          { id: 'orders' as ActivePage, label: 'My Orders', icon: History },
          { id: 'users' as ActivePage, label: 'Users', icon: Users },
          { id: 'admin' as ActivePage, label: 'Manage Products', icon: Settings },
        ]
      : []),
  ];
            ...
            {authUser ? (
              <div className="relative">
                <button
                  ...
                  title="Your account"
                >
                  <UserCircle2 className="w-3.5 h-3.5 text-[#FFD700]" />
                  <span className="hidden sm:inline max-w-[130px] truncate text-[11px] font-medium">
                    {userDisplayName}
                  </span>
                  <ChevronDown className="w-3 h-3 text-white/60" />
                </button>
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 mt-1 w-60 bg-white dark:bg-[#1b1b2d] text-[#171728] dark:text-[#FAF9F6] rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 py-2 z-50 animate-in fade-in slide-in-from-top-1">
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-xs font-semibold text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors cursor-pointer flex items-center gap-2"
                    >
              ...
              <button
                ...
                className="flex items-center gap-1.5 bg-[#0E01B5] hover:bg-[#09007A] text-white px-2.5 py-1 rounded-md text-[11px] font-bold transition-colors cursor-pointer"
                title="Sign in to your B2B account"
              >
        ...
        <div className="md:hidden bg-white dark:bg-[#171728] border-t border-gray-200 dark:border-white/10 px-4 pt-3 pb-6 space-y-4 shadow-xl animate-in slide-in-from-top-2">
          ...
          <div className="space-y-1">
            ...
            <button
              ...
              className="w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2.5 bg-emerald-600 text-white cursor-pointer"
            >
              <ShoppingCart className="w-4 h-4" />
              <span>Order from Our Catalog</span>
            </button>

            {/* Account Action (Mobile) */}
            {authUser ? (
              <button
                type="button"
                onClick={() => {
                ...
                className="w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2.5 bg-red-600 text-white"
              >
              ...
              <button
                ...
                className="w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2.5 bg-[#0E01B5] text-white"
              >
