import React, { useState } from 'react';
import { 
  ShieldCheck, 
  Info, 
  Sparkles,
  Package,
  ShoppingCart
} from 'lucide-react';
import { Product } from '../types';
import { formatKes } from '../utils/formatters';
import { getProductImageUrl, handleImageError } from '../utils/imageHelper';
import { wholesaleSavingPct } from '../utils/pricing';

interface ProductCardProps {
  product: Product;
  onOpenDetails: (product: Product) => void;
  onAddToCart: (product: Product, orderType: 'case' | 'bottle', quantity: number) => void;
}

export const ProductCard: React.FC<ProductCardProps> = ({
  product,
  onOpenDetails,
  onAddToCart,
}) => {
  const [quantity, setQuantity] = useState(1);
  return (
    <div 
      className="bg-white dark:bg-[#171728] rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden hover:border-[#0E01B5]/40 dark:hover:border-[#8c82ff]/60 hover:shadow-xl hover:-translate-y-0.5 transition-all flex flex-col justify-between group"
    >
      <div>
        {/* Product Image Area & Badges */}
        <div className="relative h-44 sm:h-48 bg-gradient-to-b from-[#FAF9F6] to-gray-100 dark:from-[#1b1b2d] dark:to-[#12121e] p-3 sm:p-4 flex items-center justify-center overflow-hidden">
          <div className="absolute inset-0 opacity-[0.04] dark:opacity-[0.06] pointer-events-none"
            style={{ backgroundImage: 'radial-gradient(circle at 20% 30%, #0E01B5 0, transparent 40%), radial-gradient(circle at 80% 70%, #F2693F 0, transparent 40%)' }}
          />
          <img
            src={getProductImageUrl(product.image)}
            alt={product.name}
            className="h-full max-h-36 sm:max-h-40 object-contain transition-transform duration-300 group-hover:scale-105"
            loading="lazy"
            referrerPolicy="no-referrer"
            onError={(e) => handleImageError(e, product.fallbackImage)}
          />

          <div className="absolute top-2.5 left-2.5 flex flex-col items-start gap-1">
            {product.kraStampVerified && (
              <div className="bg-emerald-600/90 text-white text-[10px] font-bold px-2 py-0.5 rounded-md flex items-center gap-1 shadow-sm backdrop-blur-sm">
                <ShieldCheck className="w-3 h-3" />
                KRA Stamp
              </div>
            )}

            {product.priceTier === 'wholesale' && (
              <div className="bg-[#0E01B5]/90 text-white text-[10px] font-bold px-2 py-0.5 rounded-md flex items-center gap-1 shadow-sm backdrop-blur-sm">
                <Sparkles className="w-3 h-3 text-[#FFD700]" />
                Wholesale
              </div>
            )}
          </div>

          {product.featured && (
            <div className="absolute top-2.5 right-2.5 bg-[#171728]/90 dark:bg-[#0c0c14]/90 text-[#FFD700] text-[10px] font-bold px-2 py-0.5 rounded-md flex items-center gap-1 shadow-sm backdrop-blur-sm">
              <Sparkles className="w-3 h-3 text-[#FFD700]" />
              Popular
            </div>
          )}

          <button
            type="button"
            onClick={() => onOpenDetails(product)}
            className="absolute bottom-2.5 right-2.5 bg-white/95 dark:bg-[#1f1f33]/95 hover:bg-white dark:hover:bg-[#282844] text-gray-700 dark:text-gray-200 p-1.5 rounded-lg text-xs shadow-md border border-gray-200 dark:border-gray-700 hover:text-[#0E01B5] dark:hover:text-[#8c82ff] hover:scale-105 transition-all cursor-pointer"
            title="View full specs"
          >
            <Info className="w-3.5 h-3.5" />
          </button>
        </div>

        {/* Product Details */}
        <div className="p-3.5 sm:p-4 space-y-2 sm:space-y-2.5">
          <div className="flex items-center justify-between text-[11px] text-gray-500 dark:text-gray-400">
            <span className="font-bold text-[#0E01B5] dark:text-[#8c82ff] uppercase tracking-wider text-[10px]">
              {product.brand}
            </span>
            <span className="flex items-center gap-1.5 bg-gray-100 dark:bg-[#25253d] px-1.5 py-0.5 rounded-md">
              <span>{product.originFlag}</span>
              <span className="font-semibold">{product.abv}% ABV</span>
            </span>
          </div>

          <h3 
            onClick={() => onOpenDetails(product)}
            className="font-bold text-sm sm:text-[15px] text-gray-900 dark:text-white line-clamp-1 hover:text-[#0E01B5] dark:hover:text-[#8c82ff] transition-colors cursor-pointer leading-snug"
            title={product.name}
          >
            {product.name}
          </h3>

          <div className="flex flex-wrap items-center gap-1.5 text-[10px] sm:text-[11px] text-gray-500 dark:text-gray-400">
            <span className="bg-gray-100 dark:bg-[#25253d] px-1.5 py-0.5 rounded-md text-gray-700 dark:text-gray-300 font-mono font-medium">
              SKU: {product.sku}
            </span>
            <span className="text-gray-300 dark:text-gray-600">•</span>
            <span className="flex items-center gap-1">
              <Package className="w-3 h-3 text-gray-400" />
              {product.volumeMl}ml • {product.casePack} btls/case
            </span>
          </div>

          {/* Pricing */}
          <div className="bg-[#FAF9F6] dark:bg-[#12121e] p-2.5 sm:p-3 rounded-xl border border-gray-100 dark:border-gray-800">
            <div className="flex items-baseline justify-between">
              <div>
                <span className="text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400 font-semibold uppercase tracking-wide block">
                  {product.priceTier === 'wholesale' ? 'Wholesale Case Price' : 'Normal Case Price'}
                </span>
                <span className="text-base sm:text-lg font-extrabold text-[#171728] dark:text-white tracking-tight">
                  {formatKes(product.casePriceKes)}
                </span>
              </div>
              <div className="text-right">
                <span className="text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400 font-semibold uppercase tracking-wide block">
                  Per Bottle equiv.
                </span>
                <span className="text-xs sm:text-sm font-bold text-[#686781] dark:text-gray-300">
                  {formatKes(Math.round(product.casePriceKes / product.casePack))}
                </span>
              </div>
            </div>

            {product.priceTier === 'retail' && (
              <div className="mt-1.5 pt-1.5 border-t border-gray-200/60 dark:border-gray-700 flex items-center justify-between text-[9px] sm:text-[10px]">
                <span className="text-[#0E01B5] dark:text-[#8c82ff] font-bold truncate">
                  Save {wholesaleSavingPct(product)}% with a wholesale account
                </span>
              </div>
            )}

            {product.priceTier === 'wholesale' && product.tiers.length > 0 && (
              <div className="mt-1.5 pt-1.5 border-t border-gray-200/60 dark:border-gray-700 flex items-center justify-between text-[9px] sm:text-[10px]">
                <span className="text-emerald-700 dark:text-emerald-400 font-bold flex items-center gap-1 truncate">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block shrink-0" />
                  Up to {product.tiers[product.tiers.length - 1].discountPercentage}% Bulk Discount
                </span>
                <span className="text-gray-400 font-semibold shrink-0 ml-1">
                  {product.tiers[0].minCases}+ cs
                </span>
              </div>
            )}
          </div>
        </div>
      </div>

      {/* Order Action — Add to Cart */}
      <div className="px-3.5 sm:px-4 pb-3.5 sm:pb-4 space-y-2">
        <div className="flex items-center gap-2">
          <div className="flex items-center border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-[#1b1b2d] shrink-0 overflow-hidden">
            <button
              type="button"
              onClick={() => setQuantity(Math.max(1, quantity - 1))}
              className="px-2.5 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#2e2e48] hover:text-[#0E01B5] dark:hover:text-[#8c82ff] text-xs font-bold cursor-pointer transition-colors"
            >
              −
            </button>
            <span className="px-2 text-xs font-bold min-w-[24px] text-center text-gray-900 dark:text-white">
              {quantity}
            </span>
            <button
              type="button"
              onClick={() => setQuantity(quantity + 1)}
              className="px-2.5 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#2e2e48] hover:text-[#0E01B5] dark:hover:text-[#8c82ff] text-xs font-bold cursor-pointer transition-colors"
            >
              +
            </button>
          </div>

          <button
            type="button"
            onClick={() => onAddToCart(product, 'bottle', quantity)}
            className="flex-1 py-2.5 px-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-sm bg-[#FFD700] hover:bg-[#e6c300] text-[#171728] active:scale-[0.98] cursor-pointer"
          >
            <ShoppingCart className="w-3.5 h-3.5" />
            <span>Add to Cart</span>
          </button>
        </div>
      </div>
    </div>
  );
};
